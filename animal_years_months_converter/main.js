...
function createOptionElement(type, parent, idElement, classElement,selectOptionValue, selectOptionText){
    const element = createElem(type, parent, idElement, classElement);
    element.value = selectOptionValue;
    element.text = selectOptionText;
}

function createInputRadioElement(type, inputType, inputName, parent, idElement, classElement,selectOptionValue, selectOptionText){
    const element = createElem(type, parent, idElement, classElement);
    element.value = selectOptionValue;
...
createOptionElement("option", selectAnimalElement, "", "animal-options", "", `<--Select an animal--->`)
...
animalsList.forEach(animal => {
    const option = animal;
    const animalOption = createOptionElement("option", selectAnimalElement, `${option}`, "animal-option", option, `${option}`)
})
...
sizeDescElement.innerHTML = "Small = dog under 15kg, Medium = dog from 15 to 45 kg, Big = dog over 45 kg"
...
dogSizeList.forEach(size => {
    ...
    const sizeOption = createInputRadioElement("input", "radio", "size", sizeContainerElement, `${option}`, "size-option", `${option}`, "")
})
...
function matchInputData(inputNumber, animalValuesData, unitTimeInput, animalUnitData){
    ...
    const lastAnimalValuesData = Object.keys(animalValuesData)[Object.keys(animalValuesData).length-1]
    ...
    if(lastAnimalValuesDataFormatted < inputNumber){
        ...
        error = "this value is small. Please, add larger number"
                ...
                inputNumber < firstAnimalValuesDataFormatted) ){
                    ...
                    const firstValueUnit = animalValuesData[firstAnimalValuesDataFormatted] / firstAnimalValuesDataFormatted
                    const humanYearsCal = firstValueUnit * inputNumber

                    realValueMatch.push(inputNumber)
                    realValueMatch.push(humanYearsCal)

                    console.log("this value is smaller than first value " + humanYearsCal)    
...
function calculateHumaYears(realValueMatch, maxRealValueArray, minRealValue, minHumanValue, inputNumber, realValue, humanValue){
...
function ClickBtn(){
    ...
    if(unitTimeInput ==="year" && animalUnitData === "months"){inputNumber *= 12}
    
    if(unitTimeInput ==="month" && animalUnitData === "years"){inputNumber /= 12}
   ...
   humanValue = calculateHumaYears(realValueMatch, maxRealValueArray, minRealValue, minHumanValue, inputNumber, realValue, humanValue)

    console.log("check human Value" + humanValue)
...
function getRandoDogPic(urlRandomDog){
    ...
    console.log(urlRandomDog)

    randomDogData
        .then(resp => {
            console.log("random conso " + resp.url)
            const url = resp.url;
